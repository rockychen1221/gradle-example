...
import com.littlefox.security.utils.AlgorithmUtils;
...
public class AlgorithmExecutor {
    ...
    private CrypticAlgorithm cryptic;

    /**
     * 数据执行器
     *
     */
    public AlgorithmExecutor(final CrypticAlgorithm cryptic) {
        this.cryptic = cryptic;
    }

    public static String dynamicEncrypt(String str){
        final CrypticAlgorithm dynamicCryptic = getDynamicCrypticInterface();
    ...
    public String selectMapField(String fieldValue, String type, CrypticField annotation) {
        final CrypticAlgorithm defaultCryptic = getCrypticInterface();
    ...
    public String updateMapField(String fieldValue, String type, CrypticField annotation) {
        ...
        switch (annotation.type()){
            case ONLY_ENCRYPT://可切换算法注解
                fieldValue= AlgorithmUtils.decryptSelf(fieldValue);
                break;
            default:
                    ...
                    if (field.isAnnotationPresent(CrypticField.class) && field.getType().toString().endsWith("String")) {
                        ...
                        switch (anno.type()){
                            case ONLY_ENCRYPT:
                                ...
                                field.set(t,AlgorithmUtils.decryptSelf(fieldValue));
                                break;
    ...
    private CrypticAlgorithm getCrypticInterface() {
        return cryptic;
    }

    private static CrypticAlgorithm getDynamicCrypticInterface() {
        return null;
    }
