 ...
 * @date 2018/4/4 13:36
 */
public class CryptPojo implements Cloneable, CrypticInterface {
                ...
                for (Field field : declaredFields) {
                    if (field.isAnnotationPresent(CrypticField.class) && field.getType().toString().endsWith("String")) {
                        ...
                        if (StringUtils.isNotEmpty(fieldValue)) {
                            field.set(this, CrypticUtils.getInstance().encrypt(fieldValue));
                        }
            ...
            if (declaredFields != null && declaredFields.length > 0) {
                for (Field field : declaredFields) {
                    if (field.isAnnotationPresent(CrypticField.class) && field.getType().toString().endsWith("String")) {
                        ...
                        String fieldValue = (String)field.get(this);
                        if(StringUtils.isNotEmpty(fieldValue)) {
                            field.set(this, CrypticUtils.getInstance().decrypt(fieldValue));
                        }
                    }
                }
            }
